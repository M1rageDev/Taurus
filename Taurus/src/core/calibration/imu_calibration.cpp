...
taurus::AccelCalibrator::AccelCalibrator(std::string serial, int sampleCount) {
	this->controller = ControllerManager::GetInstance()->GetController(serial);
	this->serial = serial;
	...
	this->resultScale = glm::vec3();
}

void taurus::AccelCalibrator::RunCalibration() {
	logging::info("Starting accel calibration in 2s...");
	logging::info("Place controller on stable platform, without any movement");
	psmove_util_sleep_ms(2000);
	...
	while (samples.size() < minSamples) {
		...
		// only collect if got any new data
		if (newData) {
			glm::vec3 sample = controller->GetAccel();
	...
	glm::vec3 bias = avg;
	bias.y -= 1.f;
	glm::vec3 scale = glm::vec3(1.f) / avg;
	logging::info("Accel calibration finished.");
	logging::info("Bias X: %f", bias.x);
	logging::info("Bias Y: %f", bias.y);
	logging::info("Bias Z: %f", bias.z);
	logging::info("Scale X: %f", scale.x);
	logging::info("Scale Y: %f", scale.y);
	logging::info("Scale Z: %f", scale.z);

	// save data
	json data = createAccelData(bias, scale);
	saveJson(createAccelPath(serial), data);
}
